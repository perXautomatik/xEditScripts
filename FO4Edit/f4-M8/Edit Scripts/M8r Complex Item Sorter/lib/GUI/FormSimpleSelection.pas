...
begin
	try
		...
		if options.Count > 0 then
			if Pos('=',options[0]) > 0 then
				_gui_fss_flagSpecialView := true;
				
		// Setup
		frmSimpleSel := efStartForm(nil,320,320,title);
		...
		if _gui_fss_options.Count > 0 then
			for i := 0 to _gui_fss_options.Count -1 do begin
				...
				tlab := TLabel.Create(frmSimpleSel);
		...
		// Determine form size
		frmSimpleSel.Width := 50 +20 + _gui_fss_maxLeftColWidth;
		if _gui_fss_maxRightColWidth > 0 then 
			frmSimpleSel.Width := frmSimpleSel.Width + 20 + _gui_fss_maxRightColWidth;
		
		efPadding(25,25);

		AutoForm_StartSub(frmSimpleSel, frmSimpleSel);
		AutoForm_setForm(frmSimpleSel);
		...
		if options.count > 20 then begin 
			...
			frmSimpleSel.ActiveControl  := searchEdit;
		...
		if formType = 'checkboxes' then begin 
			...
			if title = 'Record types' then begin
				...
				AutoForm_AddButton(AutoForm_getAutoTop()-10,65,30,80, 101, _('All') );
				AutoForm_AddButton(AutoForm_getAutoTop()-10,165,30,80, 102, _('None') );
				end;
			AutoForm_AddAutoTop(30);
			end;
		...
		_gui_fss_scrollbox.Width := frmSimpleSel.Width-5;
		updateEntries('');
		frm := frmSimpleSel;
		...
		tButton.Cancel := true;
		tButton.Left := (frmSimpleSel.Width - tButton.Width -10) / 2;
		tButton.tabOrder := 0;
		
		// OK Button
		if _gui_fss_formType = 'checkboxes' then begin 
			tButton.Width := 120;
			tButton.Left := (frmSimpleSel.Width - (tButton.Width*2 + 30) - 10) / 2 + tButton.Width + 30;
			tButton := AutoForm_AddButton(AutoForm_getAutoTop(),30,50,240, mrOk, #10003+' '+_('Ok') );
			tButton.Width := 120;
			tButton.Left := (frmSimpleSel.Width - (tButton.Width*2 + 30) - 10) / 2;
			tButton.tabOrder := 0;			
			end;
		
		// Final height
		frmSimpleSel.Height := AutoForm_getAutoTop() + 100;
		
		// Show form
		Result := -1;
		modalResult := -1;
		while (modalResult <> mrOk) and (modalResult <> mrCancel) do begin
			modalResult := frmSimpleSel.ShowModal();
			...
			if _gui_fss_formType = 'checkboxes' then begin 
				// Select all/none 
				if ( modalResult = 101 ) or ( modalResult = 102 ) then 
...
procedure updateEntries(searchString:String);
var 
	tButton : TButton;
	tlab: TLabel;
...
begin
	...
	// Disable form
	frmSimpleSel.DisableAlign();
	frmSimpleSel.Enabled := false;
	frmSimpleSel.Cursor := -11; // Busy
	...
	// Enable form again
	frmSimpleSel.EnableAlign();
	frmSimpleSel.Enabled := true;
	frmSimpleSel.Cursor := 0; // Default
