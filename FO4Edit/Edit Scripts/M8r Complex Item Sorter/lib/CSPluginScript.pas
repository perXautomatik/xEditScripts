...
var
	...
	_scriptFlagHasSetTagIdent: Boolean;
...
function applyScriptLines(pluginScript:TStringList;const pluginId:String;const pluginIdAndScript:String;
	var saveRecord:Boolean; var sSetNewTagIdentByScript:String;sTag:String;var madeModifications:Boolean):Boolean;
var
...
begin
	...
	_scriptFlagHasSetTagIdent := false;
	// Cache?
	if _ps_cache_enabled then begin
		cacheIndex := _ps_cache_resultsCur.indexOfName(pluginIdAndScript);
		if cacheIndex > -1 then begin
			Result := _useScriptCache(cacheIndex,pluginIdAndScript, saveRecord, madeModifications, sSetNewTagIdentByScript);
	...
	if _scriptFlagHasSetTagIdent then
		sSetNewTagIdentByScript := _scriptRuleApplyTag;
	Result := not _scriptFlagHasSetTagIdent;
...
var
	...
	lineNr, subBlockType: String;
	foundSubBlockEnd: Boolean;
begin
	Result := TStringList.Create;
	j := -1;
	for i := 0 to pluginScript.Count - 1 do begin
		...
		subBlockType := '';
		if parsedParams[0] = 'foreach' then
			subBlockType := 'foreach';
		if parsedParams[0] = 'if' then
			if parsedParams[5] = 'begin' then
				subBlockType := 'if';
		if subBlockType <> '' then begin
			blockStack := 0;
			subScriptRaw := TStringList.Create;
			foundSubBlockEnd := false;
			for j := i+1 to pluginScript.Count - 1 do begin
				if (Pos('end'+subBlockType,pluginScript.ValueFromIndex[j]) = 1 ) then
					if blockStack > 0 then
						Dec(blockStack)
					else begin
						// Found my end - Parse and store!
						_initPreparsedScriptLines(subScriptRaw, pluginIdAndScript+'>'+lineNr+'-'+subBlockType);
						// Forward script pointer
						// i := j; // Pascal doesn't like it that way .. okay ...
						foundSubBlockEnd := true;
						break;
						end;
				if (Pos(subBlockType,pluginScript.ValueFromIndex[j]) = 1 ) then
					if subBlockType = 'foreach' then 
						Inc(blockStack)
					else if subBlockType = 'if' then
						if Pos('begin', pluginScript.ValueFromIndex[j]) <> 0 then 
							Inc(blockStack);
					
				// Add line to subscript
				subScriptRaw.add(pluginScript[j])
				end;
			if not foundSubBlockEnd then 
				AddMessage('Error in plugin script: Sub block has no end: '+pluginIdAndScript+'>'+lineNr+'-'+subBlockType);
			subScriptRaw.Free;
			end;
		end;
...
begin	
	...
	if command = 'set' then
		...
		_scriptCommandIf(lineNr,params)
...
procedure _scriptCommandIf(lineNr:String;params:TStringList);
var
...
begin
	...
	if (params.Count = 6) and (params[4] = 'then' ) then begin
		...
		if compResult then begin
			...
			if paramsSub[0] = 'begin' then begin
				_scriptSubBlockStackExecute(_scriptSubBlockStackStart(lineNr+'-if'));
				_scriptSubBlockStackEnd();
				end
			else
...
function _scriptSubBlockStackStart(subBlockStackIdent:String):TStringList;
var 
	parsedParamsStorageIndex: Integer;
begin
	...
	_scriptSubBlockStack.add(subBlockStackIdent);
	parsedParamsStorageIndex := _pluginScriptParsedParamsStorage.indexOf(_scriptSubBlockStack.DelimitedText);
	Result := _pluginScriptParsedParamsStorage.Objects[parsedParamsStorageIndex];

end;

procedure _scriptSubBlockStackExecute(subScriptParsedParamsList:TStringList);
var 
	j: Integer;
begin
...
procedure _scriptSubBlockStackEnd();
begin
...
begin
	...
	// Add to exec stack
	subScriptParsedParamsList := _scriptSubBlockStackStart(lineNr+'-foreach');
	...
	if not flagIsFlags then 
		for i := 0 to elmCount - 1 do
			if not _scriptFlagDoExit then begin
				...
				// Execute sub script
				_scriptSubBlockStackExecute(subScriptParsedParamsList);
				end;
...
begin
	...
	// record entry or fixed expression
		...
		if expression = '0' then
			Result := expression
		else if {IsNumber(expression)}PregMatch('(^\d+(?:\.\d+)?$)',expression) then
			Result := expression
...
begin
	...
	if modActionIdent = 'setTagIdent' then begin
		...
		_scriptFlagHasSetTagIdent := true;
		_scriptShouldSaveRecord := true;
		Exit;
		end;
...
begin
	...
	resultPack.add(pDR_cacheValidationStr+','+sLanguageFO4EditParam);
	
	
	// Slot 1: _scriptShouldSaveRecord
	resultPack.add(_scriptShouldSaveRecord); // Boolerized
		
	// Slot 2: _scriptFlagHasSetTagIdent
	resultPack.add(_scriptFlagHasSetTagIdent); // Boolerized
...
function _useScriptCache(const cacheIndex:Integer; const pluginIdAndScript:String;var saveRecord:Boolean; var madeModifications:Boolean; var sSetNewTagIdentByScript: String;):Boolean;
var 
...
begin
	...
	// Validation 
	if (pDR_cacheValidationStr <> '' ) and (resultPack[0] = (pDR_cacheValidationStr+','+sLanguageFO4EditParam) ) then 
	...
	_scriptFlagHasSetTagIdent := resultPack[2] = 'True'; // Doppelt gemoppelt...
	if _scriptShouldSaveRecord then begin
		saveRecord := _scriptShouldSaveRecord;
		madeModifications := true;
		end;
