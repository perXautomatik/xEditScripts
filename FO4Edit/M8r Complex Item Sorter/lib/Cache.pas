...
begin
	...
	if FileExists(cachePath) then begin
		...
		// Validation		
		if cacheStorageCur.Count = 0 then 
			cacheValid := false
		else if cacheStorageCur[0] <> COMPLEX_SORTER_INTVERSION then 
			cacheValid := false
		else 
			cacheValid := true;
		if cacheValid then begin
			cacheStorageCur.Delete(0);
			...
			AddMessage('(Cache storage outdated. Renew cache.)');			
			cacheStorageCur.Clear();
			end;
		end
	else
...
begin
	...
	if Assigned(cacheStorageNew) then begin
		...
		cacheStorageNew.Insert(0,COMPLEX_SORTER_INTVERSION);
		cacheStorageNew.SaveToFile(cachePath);
		cacheStorageNew.delete(0);
...
{Removes a level-two group from the cache}
procedure removeEntrySetLevelTwo(const cacheName, cacheIdent:String);
var
	cacheIndex: Integer;
	cacheInfoPack, cacheStorageCur, cacheStorageNew: TStringList;
begin
	...
	if cacheIndex = -1 then
		Exit;
	cacheStorageCur.delete(cacheIndex);
	cacheStorageNew.delete(cacheIndex);
...
begin
	...
	// Search
	cacheIndexL1 := cacheStorageCur.indexOfName(cacheIdentL1);
	if cacheIndexL1 = -1 then begin
		AddMessage('Doesnt exist in '+cacheName+': '+cacheIdentL1+':'+cacheIdentL2);
		Exit;
		end;
	Result := true;
	sectionCacheCur := cacheStorageCur.Objects[cacheIndexL1];
	...
	// Set via index - dont trigger the hash buildup
...
begin
	if Assigned(_cCachesStorage) then
		if _cCachesStorage.Count > 0 then begin
			AddMessage('Cleanup unsaved caches..');
			for i := _cCachesStorage.Count -1 downto 0 do 
				_cleanupCache(_cCachesStorage[i]);
			end;
