unit UPF_Plugin_Loader;
	...
	function Quickloader_Finish(): null; begin
		...
		// Create the file:
			...
			File_CurrentQL := FileByName('NPCEnchantFixLoader' + IntToStr(Int_CurrentQL) + '.esp');
			if GetLoadOrder(File_CurrentQL) < 0 then File_CurrentQL := AddNewFileName('NPCEnchantFixLoader' + IntToStr(Int_CurrentQL) + '.esp',true);
			AddMessage( ' ' );

		// Set the description:
			AddMessage( 'Setting description...' );
			Entry_Decription := Add(ElementByIndex(File_CurrentQL,0), 'SNAM', false);
			SetEditValue(Entry_Decription, 'NPCEnchantFixLoader:' + Arr_SigsInQL[Int_CurrentQL]);
			AddMessage( ' ' );
