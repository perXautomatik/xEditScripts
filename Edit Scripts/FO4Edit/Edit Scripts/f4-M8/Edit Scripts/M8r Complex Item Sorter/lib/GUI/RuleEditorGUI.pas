...
begin
	...
	for i := 0 to ruleSetsToShow.Count -1 do begin
		...
		customRuleSet := CustomRuleSets.getCustomProcessingRuleSet(rulesetMainIdent,tmpLst[2]);
...
begin
	...
	efPadding(10,10);
	//tlab := efLabel('Common used',0,0,0,0,efTopAddHeight);
	...
	fButton.HelpKeyword := modifiedRulesetFullQualName;
...
begin
	...
	tmpStr := ShowFormEDIDSelection(selectedFile,selectedRecordType);
	if tmpStr <> '' then begin 
...
begin
	...
	aMenuLst.add('title=Use Assistant');
	aPresetsLst.add('');
	aMenuLst.add('Assistant=Create with new rule assistant');
...
begin
	...
	for i := 0 to tmpLst.Count -1 do begin
		...
		//tlab.Transparent := false; tlab.Color := $E1E1E1;
		_linkAction(tlab,'SET_WIN_INPUT_VALUE',nil,0,Trim(tmpLst[i]));
		efLeftAdd( 10 );
		if (efLeft > fPresetsPanel.Width) or ( tmpLst[i] = 'SPECIAL:IsArmor') then begin
			tlab.Left := 10;
...
function _winRuleAssistant(fullRule, isPrefilter:Boolean):String;
var
	i, iSel:Integer;
...
begin
	...
	if (rule <> '*') and (Pos('SPECIAL:',sElmIdent) <> 1 ) then begin
		...
		if (sElmIdent = 'KEYWORDS') or (sElmIdent = 'EFFECTS') then
			tmpLst.add('contains')
		...
		if sCompareType <> 'exists' then begin
			...
			// Compare value
			sCompareValue := 'Value';
			if not WindowPrompt('Compare value', 'Specify the value which must match. '+#10#13+#10#13+sElmIdent+'  '+sCompareType+'  "[?]"'
				+ '', sCompareValue,nil) then
				Exit;
...
begin
	...
	for i := crsModifiedRuleSets.Count -1 downto 0 do begin
		...
		// Write new rules
		customRuleSet := CustomRuleSets.getCustomProcessingRuleSet(rsMainIdent,rsSubIdent);
