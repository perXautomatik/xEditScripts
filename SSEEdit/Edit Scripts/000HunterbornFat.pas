unit UPF_Plugin_Loader;
	...
	function Quickloader_Finish(): null; begin
		...
		// Create the file:
			...
			File_CurrentQL := FileByName('HunterbornFatLoader' + IntToStr(Int_CurrentQL) + '.esp');
			if GetLoadOrder(File_CurrentQL) < 0 then File_CurrentQL := AddNewFileName('HunterbornFatLoader' + IntToStr(Int_CurrentQL) + '.esp',true);
			AddMessage( ' ' );

		// Set the description:
			AddMessage( 'Setting description...' );
			Entry_Decription := Add(ElementByIndex(File_CurrentQL,0), 'SNAM', false);
			SetEditValue(Entry_Decription, 'HunterbornFatLoader:' + Arr_SigsInQL[Int_CurrentQL]);
			AddMessage( ' ' );
		...
		// Add the masters:
			...
			for Int_LoopMasters := 0 to Pred(Arr_AddAllMasters.Count) do begin
				...
				AddMasterIfMissing(File_CurrentQL,'Hunterborn.esp');
			end;
			AddMessage( ' ' );
