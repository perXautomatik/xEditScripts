{
  ...
  v1.1:
  - Fixed bug where the script would say it failed to
    swap masters when it didn't.
  - Added new feature that allows users to remove 
    records or references which don't have a FormID
    mapping.
}
...
{
  PromptRemove:
  Prompts the user about whether or not they want to
  remove a record or reference that has no FormID
  mapping.
}
procedure PromptRemove(sName: string; e: IInterface; var sl: TStringList);
const
  sMessageBase = '%s %s has no FormID mapping.  Would you like to '+
    'remove it?  Not removing it will lead to the file %s not having '+
    'its masters swapped successfully by this script.';
  mrYes = 6;
  mrYesToAll = 10;
var
  mr: Integer;
...
begin
  ...
  if ElementType(e) = etMainRecord then begin
    // prepare and show prompt if user didn't say yes to all
    if not bRemove then begin
      f := GetFile(e);
      sMessage := Format(sMessageBase, ['Record', sName, GetFileName(f)]);
      mr := MessageDlg(sMessage, mtCustom, [mbYes, mbYesToAll, mbNo, mbNoToAll], 0);
      bRemove := (mr = mrYesToAll);
    end;
    
    // remove record
    if bRemove or (mr = mrYes) then begin
      sl.Add('        Removing record '+sName);
      Remove(e);
    end;
  end
  else begin
    // prepare and show prompt if user didn't say yes to all
    if not bRemove then begin
      f := GetFile(ContainingMainRecord(e));
      sMessage := Format(sMessageBase, ['Reference', sName, GetFileName(f)]);
      mr := MessageDlg(sMessage, mtCustom, [mbYes, mbYesToAll, mbNo, mbNoToAll], 0);
      bRemove := (mr = mrYesToAll);
    end;
    ...
    // remove record
    if bRemove or (mr = mrYes) then begin
      sl.Add('        Removing reference to '+sName+' at '+Path(e));
      Remove(e);
    end;
  end;
end;
...
begin
  for i := Pred(ElementCount(container)) downto 0 do begin
    ...
    if ElementCount(e) > 0 then begin
      ...
      if index = -1 then begin
        ...
        PromptRemove(sName, e, slOperations);
        continue;
      end;
...
begin
  ...
  try
    for i := Pred(RecordCount(f)) downto 0 do begin
      rec := records[i];
      ...
      if index = -1 then begin
        ...
        PromptRemove(sName, rec, slOperations);
        continue;
      end;
